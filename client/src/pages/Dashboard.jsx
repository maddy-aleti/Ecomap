...
function Dashboard() {
  ...
  const fetchUserReports = async () => {
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/users/reports`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
      ...
      if (response.ok) {
        ...
        const normalized = reports.map(r => ({ ...r, id: r._id || r.id }));
  ...
  const fetchDashboardStats = async () => {
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/users/dashboard`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const fetchReportVotes = async (reportId) => {
    try {
      ...
      if (response.ok) {
        ...
        // Server returns votes with field name `voteType`
  ...
  const fetchReportComments = async (reportId) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/reports/${reportId}/comments`);
      if (response.ok) {
  ...
  const handleDeleteReport = async (reportId) => {
    ...
    try {
      const token = localStorage.getItem('token') || sessionStorage.getItem('token');
      const response = await fetch(`${import.meta.env.VITE_SERVER_URL}/reports/${reportId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
